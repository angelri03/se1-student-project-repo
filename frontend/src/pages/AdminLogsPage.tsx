    ...
    <div className="min-h-screen bg-gray-900 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-6xl mx-auto">
        <div className="flex justify-between items-center mb-8">
          <button onClick={() => navigate('/explore')} className="inline-flex items-center gap-2 text-gray-400 hover:text-white">
          ...
          <h1 className="text-xl sm:text-3xl font-bold text-white">Action Logs</h1>
        ...
        <div className="mb-4 grid grid-cols-1 sm:grid-cols-4 gap-3 items-end">
          <div>
            ...
            <input value={usernameFilter} onChange={(e) => { setUsernameFilter(e.target.value); setFiltersDirty(true); }} className="w-full px-2 py-1 bg-gray-700 border border-gray-600 rounded text-white" />
          </div>
          <div>
            <label className="text-gray-300 block">Method</label>
            <input value={methodFilter} onChange={(e) => { setMethodFilter(e.target.value); setFiltersDirty(true); }} className="w-full px-2 py-1 bg-gray-700 border border-gray-600 rounded text-white" />
          </div>
          <div>
            <label className="text-gray-300 block">Path</label>
            <input value={pathFilter} onChange={(e) => { setPathFilter(e.target.value); setFiltersDirty(true); }} placeholder="path filter" className="w-full px-2 py-1 bg-gray-700 border border-gray-600 rounded text-white" />
          </div>
          <div>
            <label className="text-gray-300 block">Message Search</label>
            <input value={searchFilter} onChange={(e) => { setSearchFilter(e.target.value); setFiltersDirty(true); }} placeholder="search in message" className="w-full px-2 py-1 bg-gray-700 border border-gray-600 rounded text-white" />
          </div>
        </div>

        <div className="mb-4 grid grid-cols-1 sm:grid-cols-4 gap-3 items-end">
          <div>
            <label className="text-gray-300 block">Start (local)</label>
            <input type="datetime-local" value={startFilter} onChange={(e) => { setStartFilter(e.target.value); setFiltersDirty(true); }} className="w-full px-2 py-1 bg-gray-700 border border-gray-600 rounded text-white" />
          </div>
          <div>
            <label className="text-gray-300 block">End (local)</label>
            <input type="datetime-local" value={endFilter} onChange={(e) => { setEndFilter(e.target.value); setFiltersDirty(true); }} className="w-full px-2 py-1 bg-gray-700 border border-gray-600 rounded text-white" />
          </div>
          <div>
            <label className="text-gray-300 block">Per page</label>
            <select value={perPage} onChange={(e) => { setPerPage(Number(e.target.value)); setFiltersDirty(true); }} className="w-full px-2 py-1 bg-gray-700 border border-gray-600 rounded text-white">
          ...
          <div className="flex items-end justify-end gap-2 h-full">
            <button disabled={isFetching} onClick={async () => { setPage(1); await fetchLogs({ lines, username: usernameFilter, method: methodFilter, path: pathFilter, search: searchFilter, start: startFilter, end: endFilter, page: 1, per_page: perPage }); setFiltersDirty(false); }} className="px-3 py-1 bg-purple-600 text-white rounded disabled:opacity-50 disabled:cursor-not-allowed">Apply</button>
            <button disabled={isFetching} onClick={async () => { setUsernameFilter(''); setMethodFilter(''); setPathFilter(''); setSearchFilter(''); setStartFilter(''); setEndFilter(''); setPage(1); await fetchLogs({ lines, page:1, per_page: perPage }); setFiltersDirty(false); }} className="px-3 py-1 bg-gray-600 text-white rounded disabled:opacity-50 disabled:cursor-not-allowed">Reset</button>
          </div>
        </div>

        {message && <div className="mb-4 text-red-400">{message}</div>}

        <div className="bg-gray-800 rounded-lg shadow-xl p-4 h-[60vh] overflow-auto">
          {logs.length === 0 ? (
            <div className="text-gray-400">{filtersDirty ? 'Filters changed — click Apply to update results.' : 'No logs available.'}</div>
          ) : (
            <table className="min-w-full text-sm text-left text-gray-200">
              <thead className="text-xs text-gray-400 uppercase bg-gray-700 sticky top-0">
                <tr>
                  <th className="px-3 py-2">
                    <button onClick={toggleSortTime} className="inline-flex items-center gap-2 text-gray-300 hover:text-white">
                      <span>Time</span>
                      {sortDir === 'asc' ? (
              ...
              <tbody>
                {logs.map((l, idx) => (
                  <tr key={idx} className={idx % 2 === 0 ? 'bg-gray-800' : 'bg-gray-900'}>
                    ...
                    <td className="px-3 py-2 align-top"><div className="whitespace-pre-wrap text-xs text-gray-300">{l.message}</div></td>
                  </tr>
                ))}
              </tbody>
            </table>
